...
/// Time Of Impact of a composite shape with any other shape, under translational movement.
pub fn composite_shape_against_shape_with_normal<P, M, G1: ?Sized>(m1: &M, vel1: &P::Vect, g1: &G1,
...
/// Time Of Impact of any shape with a composite shape, under translational movement.
pub fn shape_against_composite_shape_with_normal<P, M, G2: ?Sized>(m1: &M, vel1: &P::Vect, g1: &Shape<P, M>,
...
struct CompositeShapeAgainstAnyTOINormalCostFn<'a, P: 'a + Point, M: 'a, G1: ?Sized + 'a> {
    msum_shift:  P::Vect,
    msum_margin: P::Vect,
    ray:         Ray<P>,
...
impl<'a, P, M, G1: ?Sized> CompositeShapeAgainstAnyTOINormalCostFn<'a, P, M, G1>
    where P:  Point,
          M:  Isometry<P>,
          G1: CompositeShape<P, M> {
    pub fn new(m1: &'a M, vel1: &'a P::Vect, g1: &'a G1, m2: &'a M, vel2: &'a P::Vect, g2: &'a Shape<P, M>)
        -> CompositeShapeAgainstAnyTOINormalCostFn<'a, P, M, G1> {

        let ls_m2 = na::inverse(m1).expect("The transformation `m1` must be inversible.") * *m2;
        let ls_aabb2 = g2.aabb(&ls_m2);

        CompositeShapeAgainstAnyTOINormalCostFn {
...
for CompositeShapeAgainstAnyTOINormalCostFn<'a, P, M, G1>
    where P:  Point,
          ...
          M:  Isometry<P>,
          G1: CompositeShape<P, M> {
    type UserData = (<P::Vect as Vector>::Scalar, P::Vect);
    ...
    #[inline]
    fn compute_b_cost(&mut self, b: &usize) -> Option<(<P::Vect as Vector>::Scalar, Self::UserData)> {
        let mut res = None;

        self.g1.map_transformed_part_at(*b, self.m1, &mut |m1, g1|
            res = time_of_impact_internal::time_of_impact_with_normal(m1, self.vel1, g1, self.m2, self.vel2, self.g2)
                  .map(|(toi, normal)| (toi, (toi, normal)))
        );
